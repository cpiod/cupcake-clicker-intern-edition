  ...
  for e in all(_ents) do
   if has(e,"machine") then
    if e.nb==nb_m then
     ...
     if(has(e,"running")) nice_print((ceil(100*60*e.quality/e.time)/100).."$/s",12+128*nb_m,60,12)
     break
    end
   end
  end
...
function draw_search()
 ...
 fillp()
 x_sliding=-105
 local wait=day<search_day
 if t12 and day>=search_day then
  x_sliding=-105+50*(t()-.8-t12)
  printh(-105+50*(t()-.8-t12))
...
--  ?"slide to open",x+5,50,6
  if mode==12 then
   ...
   if day<search_day then
    ?"closed until 7/10",x_sliding+5,60,5
   end
  end
 ...
 if left==0 then
  if current<target then
   ...
   day+=1
	...
	lclick=btnp(5)
	rclick=btnp(4)
	if mode>=10 and mode<20 and not search_found then
	 produce()
	 left-=1
	end
	if mode==0 then
...
function update_color(lclick)
 -- also used in draw
  ...
  if mouse_x>=7 and mouse_x<=15 and mouse_y>=71 and mouse_y<79 then
   change_mode(10)
   return
  end
 end
...
function get_yield(recipe)
 printh(recipe.coeff.." "..recipe.quality)
 return ceil(recipe.coeff*100*60*recipe.quality/recipe.time)/100
end
-->8
-- search
...
function update_search()
 ...
 if mouse_x+camera_x<x_sliding and dx*dx+dy*dy<6 and not search_found then
  search_found=true
  id=rnd(rewards)
  flash=id -- by default
  printh("reward:"..id)
